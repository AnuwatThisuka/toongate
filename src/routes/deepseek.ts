...
const deepseek = new Hono<{ Bindings: Env }>();

const DEEPSEEK_DEFAULT_UPSTREAM = "https://api.deepseek.com";

function buildUpstreamUrl(endpoint: string, env: Env): string {
  const base = (env.DEEPSEEK_UPSTREAM_URL || DEEPSEEK_DEFAULT_UPSTREAM).replace(/\/$/, "");
  // Strip /deepseek prefix → /v1/chat/completions or /v1/embeddings
  const path = endpoint.replace(/^\/deepseek/, "");
  return base + path;
}
...
function buildUpstreamHeaders(c: Context<{ Bindings: Env }>): Headers {
  ...
  headers.set("Authorization", `Bearer ${c.env.DEEPSEEK_API_KEY}`);
...
async function proxy(
  ...
  if (!c.env.DEEPSEEK_API_KEY && !c.env.DEEPSEEK_UPSTREAM_URL) {
    return c.json({ error: "DeepSeek is not configured" }, 503);
  }
  ...
  if (env.TOON_DRY_RUN === "true" && result.compressed) {
    console.log(
      `[TOON DRY-RUN] provider=deepseek model=${model} endpoint=${endpoint}` +
...
deepseek.post("/deepseek/v1/chat/completions", (c) =>
  proxy(c, "/deepseek/v1/chat/completions"),
);
deepseek.post("/deepseek/v1/embeddings", (c) =>
  proxy(c, "/deepseek/v1/embeddings"),
);
