...
toongate Worker
   ...
   ├── 2. Score payload eligibility (0–1)
   ├── 3. Compress if score ≥ threshold
   ├── 4. Forward to upstream (gateway or provider)
   ├── 5. Pass response back to your app
   └── 6. Write savings row to D1 (fire-and-forget)
   ▼
...
```
Your App → toongate → compress request → upstream
                             ↓
Your App ← SSE chunks ← upstream (streamed live)
                             ↓
                    write savings to D1 after [DONE]
```
