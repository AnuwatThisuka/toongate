...
sequenceDiagram
    ...
    TG->>TG: 3. Top-level compress if score ≥ threshold
    TG->>TG: 4. Deep compress JSON arrays in content[].text (if step 3 skipped)
    TG->>US: 5. Forward (compressed) payload
    US-->>TG: Response
    TG-->>App: 6. Pass response back (standard JSON)
    TG-)D1: 7. Write savings row (fire-and-forget)
```
...
| JSON string in `content[].text` is not a valid array | Skip silently, no deep compress |
| Deep-compressed payload not smaller than original | Fall back to original payload |
